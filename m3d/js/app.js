...
async function renderPlus() {
  ...
  document.getElementById("resetCotisBtn").addEventListener("click", async () => {
    const ok = await confirmWithPassword("Ceci va supprimer TOUS les dimanches, paiements, cadeaux verses et remboursements pour repartir a zero. Cette action est definitive. Confirme avec le mot de passe administrateur.");
    if (!ok) return;
    await reinitialiserCotisations();
    toast("Cotisations reinitialisees");
    showTab("plus");
  });
}
...
async function importBackup(e) {
  ...
  if (!file) return;
  const ok = await confirmWithPassword("Importer va REMPLACER toutes les donnees actuelles. Confirme avec le mot de passe administrateur pour continuer.");

// ============================================================================
// app.js — Routeur + toutes les vues de l'application. Vanilla JS pur, sans
// framework (React/Vue) : choix delibere pour rester simple, leger et 100%
// comprehensible sans etape de build. Depend de db.js (couche donnees,
// charge avant ce fichier dans index.html) et des librairies vendorisees
// dans js/vendor/ (Dexie, XLSX) — jamais de CDN externe, voir plus bas.
// ============================================================================
//
// COMMENT LIRE CE FICHIER (pour retrouver quelque chose rapidement) :
// Chaque grande section est delimitee par un bandeau "// ---...---" avec
// un titre. Dans l'ordre : utilitaires generaux, menus deroulants, theme
// clair/sombre, authentification (mot de passe admin), navigation entre
// onglets (showTab), recherche globale, tableau de bord (Accueil),
// module Membres, module Dimanches (collectes), module Dettes, module
// "Mes listes" (evenements independants), module Plus (parametres/
// exports/zone dangereuse), exports PDF/Excel/Word, sauvegarde JSON.
//
// REGLE DE SECURITE #1 — A NE JAMAIS OUBLIER :
// Toute donnee qui vient d'un formulaire ou d'un import JSON (nom,
// telephone, fonction, libelle, notes, nom de liste...) DOIT passer par
// esc() avant d'etre inseree dans un template `...${xxx}...` utilise avec
// innerHTML OU dans une fenetre d'impression (writePrintableDocument, qui
// utilise document.write — c'est du DOM EN DIRECT, encore plus sensible
// qu'un simple innerHTML). Voir la definition de esc() juste en dessous
// pour le detail. Une regression sur ce point s'est deja produite une
// fois (fonction esc() disparue en repartant d'une ancienne version) —
// si un jour l'app affiche a nouveau du HTML non echappe, chercher
// d'abord si esc() existe toujours et si elle est bien appelee partout.
//
// REGLE DE SECURITE #2 — COMPATIBILITE iOS 12 (iPad mini 2 notamment) :
// L'appareil cible le plus ancien ne supporte NI l'optional chaining
// (?.) NI le nullish coalescing (??) — ce sont des fonctionnalites
// Safari 13.4+. Une seule utilisation de ces operateurs QUELQUE PART
// dans ce fichier empeche le navigateur de PARSER le fichier entier :
// resultat, l'app entiere reste blanche sans aucune erreur visible pour
// l'utilisateur. crypto.randomUUID() est egalement absent avant Safari
// 15.4 (voir uid() dans db.js qui gere deja ce cas). Avant tout ajout de
// code, verifier avec `grep -n '??\|?\.' js/app.js js/db.js`.
// ============================================================================
...
// ================================================================
// A QUOI CA SERT :
// Toutes les valeurs qui viennent d'un formulaire ou d'un import JSON
// (nom, prenom, telephone, fonction, libelle de mouvement, nom de liste,
// notes, recherche...) doivent passer par esc() avant d'etre inserees
// dans un template `html` utilise avec innerHTML.
//
...
// JSON trafique), ce code s'execute directement des que le nom s'affiche
// n'importe ou dans l'app (dashboard, liste membres, dimanches, dettes,
// mouvements, listes/evenements, exports). C'est une faille XSS stockee.
//
// REGLE A SUIVRE POUR TOUTE NOUVELLE FONCTIONNALITE :
// Des que tu ecris `${maVariable}` a l'interieur d'un template litteral
// qui finit dans innerHTML (ou dans un attribut value="${...}"), demande-
// toi : "est-ce que maVariable vient d'un utilisateur (formulaire, import
// JSON) ?" Si oui -> `${esc(maVariable)}`. Si c'est un nombre/une date
// deja formatee par fmt()/fmtDate() -> pas besoin, ce n'est pas du texte
// libre.
const esc = (s) =>
...
// authOverlay : construit l'ecran plein-ecran (mot de passe / creation de
// mot de passe) qui recouvre TOUTE l'app tant que authed est false. Ce
// n'est PAS une vraie barriere de securite cote serveur (il n'y a pas de
// serveur, tout est local) : c'est une protection d'ecran pour eviter
// qu'une personne qui prend l'iPad en main tombe directement sur les
// donnees. Les vraies donnees sensibles (mot de passe) restent quand
// meme protegees par le hash SHA-256 sale (voir db.js).
...
// ---------------------------------------------------------------
// Recherche globale (barre de recherche accessible depuis le dashboard,
// cherche a la fois dans les membres et dans les listes/evenements)
// ---------------------------------------------------------------
  ...
  box.innerHTML =
    section(
      ...
      matchMembres
        .map(
          (m) =>
            `<div class="gsr-item" data-go="membre" data-id="${m.id}"><span class="avatar" style="width:28px;height:28px;font-size:11px;">${esc(initials(m))}</span>${esc(fullName(m))}</div>`,
        )
        .join(""),
    ) +
    section(
    ...
    section(
      ...
      matchCotis
        .map(
          (j) =>
            `<div class="gsr-item" data-go="dimanche" data-id="${j.dimanche.id}">${fmtDate(j.dimanche.date)} — ${j.beneficiaires.join(", ") || "Collecte"}</div>`,
        )
        .join(""),
    );
  box.classList.add("show");
...
// renderAccueil : construit le tableau de bord principal (1er onglet).
// Rassemble en un seul rendu : KPI du haut (total collecte, dettes,
// solde caisse), prochains anniversaires, anniversaires du mois en
// cours, graphiques (caisse/mois/donut du dernier dimanche) et le
// recapitulatif des derniers dimanches de collecte. C'est la vue la
// plus lourde de l'app (le plus de requetes db.js en parallele) : si
// le dashboard devient lent avec beaucoup de donnees, c'est le premier
// endroit a profiler.
async function renderAccueil() {
  ...
  document.getElementById("prochainsBox").innerHTML =
    prochains
      ...
      <div class="avatar">${esc(initials(x.membre))}</div>
  ...
  document.getElementById("moisBox").innerHTML =
    ...
    <div class="row" data-id="${m.id}">
      <div class="avatar" style="background:var(--bg-warning);color:var(--warning);">${esc(initials(m))}</div>
...
// weekCardDetailedHTML / weekCardHTML : deux gabarits d'affichage pour
// une carte "dimanche de collecte". La version "Detailed" (utilisee sur
// le dashboard) montre en plus qui n'a pas cotise. La version courte
// (weekCardHTML, module Dimanches) est plus compacte pour une liste
// longue. Les deux DOIVENT echapper les noms avec esc() — voir plus
// haut la regle de securite #1.
function weekCardDetailedHTML(j, memById) {
  ...
  const who = j.beneficiaires.length
    ? j.beneficiaires.join(", ")
    : "Aucun anniversaire cette semaine";
  const nonPayeurs = j.paiements
    .filter((p) => !p.a_paye)
    .map((p) => (memById[p.id_membre] ? fullName(memById[p.id_membre]) : "?"));
  return `<div class="week-card" data-dimanche="${j.dimanche.id}">
    ...
    ${nonPayeurs.length ? `<div class="detail-row" style="border:none;padding:0 0 6px;"><span class="k">N'ont pas cotise</span><span class="v" style="color:var(--danger);text-align:right;">${esc(nonPayeurs.join(", "))}</span></div>` : ""}
...
// ---------------------------------------------------------------
// Module Membres — liste, recherche, filtres, fiche detail, edition.
// memberQuery/memberSort/memberFilterXxx sont des variables MODULE
// (pas dans une fonction) : elles persistent tant que l'app reste
// ouverte, pour que la recherche/le tri restent actifs si on navigue
// vers une fiche puis qu'on revient a la liste.
// ---------------------------------------------------------------
  ...
  box.innerHTML =
    list
      .map(
        (m) => `
    <div class="row" data-id="${m.id}">
      <div class="avatar" style="${m.statut === "Inactif" ? "opacity:.45" : ""}">${esc(initials(m))}</div>
...
// openMemberDetail : ouvre la fiche complete d'un membre dans un sheet
// (panneau glissant). Toutes les valeurs affichees qui viennent du
// membre (nom, telephone, fonction, observations) passent par esc().
...
// ---------------------------------------------------------------
// Module Dimanches — historique des collectes du dimanche, creation
// d'un nouveau dimanche, detail d'un dimanche (qui a paye, qui doit
// encore payer, marquer un paiement).
// ---------------------------------------------------------------
...
// openNewSunday : formulaire de creation d'un nouveau dimanche de
// collecte. Propose par defaut les membres suggeres par
// membresAnniversaireCeDimanche() (db.js) — mais l'utilisateur peut
// cocher/decocher manuellement avant de valider (cas d'un anniversaire
// oublie dans les dates, ou d'une collecte exceptionnelle).
...
// openWeekDetail : detail d'un dimanche precis — bascule chaque
// paiement individuel (paye/non paye) via marquerPaiement (db.js), et
// permet de modifier la date ou de supprimer le dimanche entier.
async function openWeekDetail(dimId) {
  ...
  const benefNames =
    anniv
      .map((a) =>
        memById[a.id_membre_fete] ? fullName(memById[a.id_membre_fete]) : "?",
      )
...
// ---------------------------------------------------------------
// ---------------------------------------------------------------
// Module Dettes — liste des paiements impayes (dettesList() de db.js),
// avec possibilite de marquer un remboursement. Rappel : une dette
// n'est jamais stockee, elle est deduite des paiements a_paye=false.
// ---------------------------------------------------------------
async function renderDettes() {
  ...
  const rowHTML = (d, actionable) => `
    <div class="row" ${actionable ? `data-paiement="${d.id_paiement}"` : ""}>
      <div class="avatar" style="background:var(--bg-danger);color:var(--danger);">${esc((d.membre[0] || "?").toUpperCase())}</div>
...
// openRembourser : confirme le remboursement d'UNE dette precise.
// Cree une ligne dans remboursements (ne modifie PAS le paiement
// original, pour garder une trace historique complete de ce qui
// s'est passe : impaye a l'origine, puis rembourse tel jour).
...
// ---------------------------------------------------------------
// Module "Mes listes" — evenements/groupes independants des
// cotisations d'anniversaire (sorties, reunions, camps...). Chaque
// liste a des membres inscrits avec un statut present/absent/attente.
// listeIconSVG fait une simple lookup dans LISTE_ICONES par cle connue
// (jamais de HTML libre insere ici, donc pas besoin d'echapper icone).
// ---------------------------------------------------------------
...
async function renderListesList() {
  ...
  const cardsHtml = await Promise.all(
    filtered.map(async (l) => {
      ...
      return `<div class="card liste-card" data-id="${l.id}" style="border-left:4px solid ${esc(l.couleur)};">
      <div class="liste-card-top">
        <span class="liste-icon" style="background:${esc(l.couleur)}22;color:${esc(l.couleur)};">${listeIconSVG(l.icone)}</span>
...
// openCreerListe : formulaire de creation d'une liste. couleur/icone
// sont choisies parmi des PALETTES FIXES (LISTE_COULEURS/LISTE_ICONE_
// KEYS, pas de saisie libre dans l'UI normale) — mais on echappe quand
// meme couleur a l'affichage (esc(l.couleur)) car un import JSON
// trafique pourrait contenir une valeur arbitraire dans ce champ.
...
async function renderListeDetailSheet(id) {
  ...
  const html = `
    <button class="sheet-close" data-close>&times;</button>
    <div class="liste-detail-head" style="border-left:4px solid ${esc(l.couleur)};padding-left:12px;">
      <span class="liste-icon" style="background:${esc(l.couleur)}22;color:${esc(l.couleur)};">${listeIconSVG(l.icone, 22)}</span>
...
// refreshListeDetailBody : redessine uniquement le CONTENU de la fiche
// liste (pas tout le sheet) apres une action (ajout/retrait membre,
// changement de presence) — evite de fermer/rouvrir tout le panneau.
    ...
    addResultsBox.innerHTML = candidats.length
      ? `<div class="small-note" style="margin:4px 0;">Ajouter a la liste :</div>` +
        ...
        <div class="row" data-add="${m.id}" style="cursor:pointer;">
          <div class="avatar">${esc(initials(m))}</div>
...
// ---------------------------------------------------------------
// Exports PDF (fenetre imprimable) et Excel d'une liste precise.
// ATTENTION : exportListePDF passe par writePrintableDocument, donc
// document.write() sur une fenetre reelle du navigateur — toute
// donnee utilisateur (nom de liste, description, notes, noms des
// membres) DOIT etre echappee avec esc() ici, contrairement a
// exportListeExcel qui remplit un tableau de donnees (XLSX) et n'a pas
// ce risque (les cellules Excel n'executent pas de HTML/JS).
// ---------------------------------------------------------------
async function exportListePDF(id) {
  ...
  `;
  if (win) writePrintableDocument(win, esc(l.nom), body);
}
...
// ---------------------------------------------------------------
// ---------------------------------------------------------------
// Module Plus — parametres (montants par defaut), theme, mouvements
// de caisse manuels, exports, sauvegarde JSON, et la "Zone dangereuse"
// (reinitialisations). Voir reinitialiserCotisations() dans db.js pour
// le detail exact de ce qui est efface/conserve par chaque bouton.
// ---------------------------------------------------------------
async function renderPlus() {
  const solde = await caisseSolde();
  ...
  app.innerHTML = `
    ...
    <div class="card" style="margin-bottom:24px;border-color:var(--danger);">
      ...
      <div class="small-note">Remet a zero tous les paiements, les dettes et les mouvements de caisse manuels. La liste des dimanches de collecte (dates, anniversaires du jour) N'EST PAS supprimee : chaque dimanche repasse simplement a 0/0 cotisation. Les membres ne sont pas touches.</div>
    </div>
  `;
  ...
  document
    ...
    .addEventListener("click", async () => {
      const ok = await confirmWithPassword(
        "Ceci va remettre a zero TOUS les paiements, les dettes et les mouvements de caisse manuels. La liste des dimanches (dates, anniversaires du jour) sera conservee. Cette action est definitive. Confirme avec le mot de passe administrateur.",
      );
      if (!ok) return;
      await reinitialiserCotisations();
      toast("Cotisations, dettes et caisse reinitialisees");
      showTab("plus");
    });
}
...
// de Safari.
// ---------------------------------------------------------------
// Exports PDF (impression navigateur) — communs a tous les modules.
// openPrintableWindow ouvre un onglet vide, writePrintableDocument y
// ecrit le HTML final puis lance l'impression automatiquement (avec
// un delai de secours car certains navigateurs, dont d'anciens
// Safari, declenchent onload avant que document.write() soit pret).
//
// RAPPEL SECURITE : win.document.write() ecrit dans une VRAIE fenetre
// de navigateur avec JS actif. Toute chaine passee ici (title,
// bodyHtml) qui contient une donnee utilisateur non echappee est une
// faille XSS reelle, pas juste theorique. Chaque fonction exportXxxPDF
// plus bas doit passer ses donnees utilisateur (noms, libelles,
// notes...) dans esc(...) AVANT de les mettre dans le template `body`.
// ---------------------------------------------------------------
...
// exportMembresPDF : liste complete des membres, format impression.
...
// exportCotisationPDF : detail d'un dimanche precis (qui a paye,
// combien, qui reste a relancer) en version imprimable.
...
// exportRapportPDF : le "gros" rapport — reprend integralement les
// donnees de rapportStats() (db.js) : resume general, repartition par
// fonction/mois, membres les plus reguliers/absents, historique des
// 20 derniers dimanches. Si on ajoute une nouvelle stat a
// rapportStats(), penser a l'ajouter aussi ici pour qu'elle apparaisse
// dans le PDF (les deux ne sont pas synchronises automatiquement).
async function exportRapportPDF() {
  ...
  const histRows = r.joursStats
    ...
    .map(
      (j) =>
        `<tr><td>${fmtDate(j.dimanche.date)}</td><td>${j.beneficiaires.join(", ") || "—"}</td><td>${fmt(j.totalCollecte)}</td><td>${j.nbPayants}/${j.nbTotal}</td></tr>`,
    )
    .join("");
  const body = `
...
// ---------------------------------------------------------------
// Exports Excel (.xlsx via SheetJS/XLSX, vendorise localement dans
// js/vendor/). Contrairement aux exports PDF, PAS besoin d'echapper
// avec esc() ici : XLSX.utils.json_to_sheet() met les valeurs dans des
// cellules de tableur, qui n'executent jamais de HTML/JS — echapper
// quand meme afficherait des '&amp;' etc. au lieu du texte normal
// dans Excel, ce qui serait un bug d'affichage, pas une securite.
// ---------------------------------------------------------------
...
// exportRapportWord : genere un fichier .doc en ecrivant du HTML brut
// avec l'extension .doc (Word l'ouvre et l'interprete comme du HTML).
// C'est un FICHIER TELECHARGE, pas une fenetre live comme les exports
// PDF — le risque XSS direct est donc plus faible, mais on echappe
// quand meme les donnees utilisateur par coherence et parce que si le
// fichier est un jour renomme en .html et ouvert dans un navigateur,
// le risque redevient reel.
...
// ---------------------------------------------------------------
// ---------------------------------------------------------------
// Sauvegarde / restauration JSON — LE moyen officiel de transferer les
// donnees d'un appareil a l'autre (l'app est 100% locale, IndexedDB
// n'est jamais synchronise automatiquement entre appareils).
// ---------------------------------------------------------------
// exportBackup : telecharge un fichier JSON avec le contenu ACTUEL de
// toutes les tables IndexedDB. A faire regulierement (surtout sur
// iPad iOS ancien : voir la note dans importBackup ci-dessous sur la
// fiabilite du stockage local sur les vieux Safari).
...
// importBackup : restaure une sauvegarde JSON, en ECRASANT toutes les
// donnees actuelles (confirmation par mot de passe admin obligatoire
// avant d'ecraser quoi que ce soit).
//
// SECURITE : on ne fait PAS confiance a la structure du fichier
// importe. KNOWN_TABLES est une LISTE BLANCHE des tables reconnues du
// schema (db.js) : toute cle presente dans le JSON mais absente de
// cette liste est ignoree, et chaque table doit contenir un tableau
// (sinon ignoree aussi). Ca empeche un fichier de sauvegarde trafique
// d'ecrire n'importe quoi dans IndexedDB. Si on ajoute une nouvelle
// table dans db.js (comme "listes"/"liste_membres" l'ont ete), il FAUT
// l'ajouter aussi a KNOWN_TABLES ci-dessous, sinon l'import silencieux
// ignorera cette table sans prevenir personne.
async function importBackup(e) {
  const file = e.target.files[0];
  if (!file) return;
  const ok = await confirmWithPassword(
    "Importer va REMPLACER toutes les donnees actuelles. Confirme avec le mot de passe administrateur pour continuer.",
  );
  if (!ok) {
    e.target.value = "";
    return;
  ...
  // chaque fois qu'une table est ajoutee au schema.
  const KNOWN_TABLES = [
    "membres",
    "sessions",
    "dimanches",
  ...
  try {
    ...
    const tables = Object.keys(parsed.data).filter((t) => {
      if (KNOWN_TABLES.indexOf(t) === -1) return false;
      if (!Array.isArray(parsed.data[t])) return false;
      return true;
    });
    if (tables.length === 0)
